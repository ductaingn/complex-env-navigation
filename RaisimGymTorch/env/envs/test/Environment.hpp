...
namespace raisim
{
    ...
    public:
        ...
        : RaisimGymEnv(resourceDir, cfg), visualizable_(visualizable)
        {
            ...
            if (analytic_planner_run) {
                husky_sphere_ = world_->addSphere(0.55, 1, "default", raisim::COLLISION(2), RAISIM_STATIC_COLLISION_GROUP);
    ...
    {
        husky_sphere_->setPosition(0., 0., 0.);
    ...
    void updateObservation()
    {
        ...
        obDouble_ << rot.e().row(2).transpose(),      /// body orientation (dim=3)
                gc_.tail(nJoints_),                    /// joint angles (dim=12)
                bodyLinearVel_, bodyAngularVel_, /// body linear&angular velocity (dim=3+3=6)
                gv_.tail(nJoints_),                  /// joint velocity (dim=12)
