...
namespace raisim {
    ...
    public:
        explicit ENVIRONMENT(const std::string &resourceDir, const Yaml::Node &cfg, bool visualizable,
            ...
            std::uniform_real_distribution<> obstacle_randomness_sampler(min_obstacle_randomness,
                                                                         max_obstacle_randomness);
            ...
            if (env_type == 1) {
                ...
                std::uniform_real_distribution<> env_one_grid_size_sampler(min_obstacle_grid_size,
                                                                           max_obstacle_grid_size);
                double obstacle_grid_size = env_one_grid_size_sampler(generator);
                ...
                std::uniform_real_distribution<> obstacle_center_sampler(obstacle_randomness,
                                                                         obstacle_grid_size - obstacle_randomness);
                ...
                /// generate obstacles
                double min_obstacle_size = cfg["obstacle_size"]["env_one"]["min"].As<double>();
                double max_obstacle_size = cfg["obstacle_size"]["env_one"]["max"].As<double>();
                    ...
                    for (int i = 0; i < hm_samplesX; i++) {
                        ...
                        for (int k = 0; k < n_obstacle; k++) {
                            ...
                            if (obstacle_type_list[k] == 1) {
                                ...
                                if (sqrt(pow(x - obstacle_x, 2) + pow(y - obstacle_y, 2)) <
                                    (obstacle_circle_dr[k] + 0.7))
                                    available_init = false;
                            } else {
                                // box obstacle
                                if (abs(x - obstacle_x) <= obstacle_box_size[k] / 2 &&
                                    abs(y - obstacle_y) <= obstacle_box_size[k] / 2)
                                    available_obstacle = true;
                                if (sqrt(pow(x - obstacle_x, 2) + pow(y - obstacle_y, 2)) <
                                    ((obstacle_box_size[k] * sqrt(2)) / 2 + 0.7))
                                    available_init = false;
                            }
                        }
                ...
                double obstacle_corridor_short = uniform_obstacle_short(generator);
                double obstacle_corridor_long = uniform_obstacle_long(generator);
                ...
                /// set parameter limits of obstacle
                double min_obstacle_grid_size, max_obstacle_grid_size, min_obstacle_size, max_obstacle_size;
                if (point_goal_initialize || CVAE_data_collection_initialize || CVAE_environment_initialize) {
                    // different parameter limits because goal should exist
                    min_obstacle_grid_size = 3.0;
                    max_obstacle_grid_size = 5.0;
                    min_obstacle_size = 0.05;
                    max_obstacle_size = 0.5;
                } else {
                    min_obstacle_grid_size = cfg["obstacle_grid_size"]["env_two"]["min"].template As<double>();
                    max_obstacle_grid_size = cfg["obstacle_grid_size"]["env_two"]["max"].template As<double>();
                    min_obstacle_size = cfg["obstacle_size"]["env_two"]["min"].As<double>();
                    max_obstacle_size = cfg["obstacle_size"]["env_two"]["max"].As<double>();
                }

                /// sample obstacle center for cross-corridor
                std::uniform_real_distribution<> env_two_grid_size_sampler(min_obstacle_grid_size,
                                                                           max_obstacle_grid_size);
                double obstacle_grid_size = env_two_grid_size_sampler(generator);
                int n_x_grid = int(hm_sizeX / obstacle_grid_size);
                int n_y_grid = int(hm_sizeY / obstacle_grid_size);
                n_obstacle = n_x_grid * n_y_grid;
                ...
                for (int i = 0; i < n_obstacle; i++) {
                    int current_n_y = int(i / n_x_grid);
                    int current_n_x = i - current_n_y * n_x_grid;
                    double sampled_x = obstacle_center_sampler(generator);
                    double sampled_y = obstacle_center_sampler(generator);
                    sampled_x += obstacle_grid_size * current_n_x;
                    sampled_x -= hm_sizeX / 2;
                    sampled_y += obstacle_grid_size * current_n_y;
                    sampled_y -= hm_sizeY / 2;
                ...
                for (int j = 0; j < hm_samplesY; j++) {
                    for (int i = 0; i < hm_samplesX; i++) {
                        ...
                        if (obstacle_idx_small + 0.7 < x && x < obstacle_idx_big - 0.7)
                            available_init = true;
                        if (obstacle_idx_small + 0.7 < y && y < obstacle_idx_big - 0.7)
                            available_init = true;
            ...
            if (point_goal_initialize || CVAE_data_collection_initialize || CVAE_environment_initialize) {
                try {
                    ...
                    if (one_square_goal.size() < 2 || two_square_goal.size() < 2 || three_square_goal.size() < 2 ||
                        four_square_goal.size() < 2) {
                        throw true;
                    } else {
            ...
            /// add heightmap
            hm = world_->addHeightMap(hm_samplesX, hm_samplesY, hm_sizeX, hm_sizeY, hm_centerX, hm_centerY,
                                      hm_raw_value);
            ...
            if (visualizable_) {
                ...
                for (int i = 0; i < n_prediction_step; i++) {
                    desired_command_traj.push_back(
                            server_->addVisualBox("desired_command_pos" + std::to_string(i), 0.08, 0.08, 0.08, 1, 1,
                                                  0));  // yellow
                    modified_command_traj.push_back(
                            server_->addVisualBox("modified_command_pos" + std::to_string(i), 0.08, 0.08, 0.08, 0, 0,
                                                  1));  // blue
                }
                ...
                if (analytic_planner_run) {
                    for (int i = 0; i < n_analytic_planner_waypoints; i++) {
                        analytic_planner_waypoints.push_back(
                                server_->addVisualCylinder("analytic_waypoint" + std::to_string(i), 0.4, 0.8, 0, 1, 1));
                    }
                }
...
//        static std::default_random_engine generator(random_seed);
            ...
            if (random_initialize || CVAE_environment_initialize) {
                ...
                if (current_n_step == 0) {
                    if (env_type == 1) {
                        // position closest to the center
                        for (int i = 0; i < 2; i++)
        ...
        void updateObservation() {
            ...
            for (int i = 0; i < scanSize; i++) {
                ...
                if (col.size() > 0) {
                    ...
                    lidar_scan_depth[i] =
                            std::max(std::min(current_lidar_distance + lidar_noise_distance, ray_length), 0.) /
                            ray_length;
                } else {
                    if (visualizable_)
        ...
        void updateHistory(Eigen::VectorXd current_joint_position_error,
            ...
            for (int i; i < n_history_steps - 1; i++) {
                joint_position_error_history.segment(i * nJoints_, nJoints_) = joint_position_error_history.segment(
                        (i + 1) * nJoints_, nJoints_);
                joint_velocity_history.segment(i * nJoints_, nJoints_) = joint_velocity_history.segment(
                        (i + 1) * nJoints_, nJoints_);
            }
        ...
        void visualize_desired_command_traj(Eigen::Ref<EigenRowMajorMat> coordinate_desired_command,
            ...
            for (int i = 0; i < n_prediction_step; i++) {
                ...
                desired_command_traj[i]->setPosition(
                        {coordinate_desired_command(i, 0), coordinate_desired_command(i, 1), coordinate_z});
        ...
        void visualize_modified_command_traj(Eigen::Ref<EigenRowMajorMat> coordinate_modified_command,
            ...
            for (int i = 0; i < n_prediction_step; i++) {
                ...
                const double coordinate_z = 0;
                modified_command_traj[i]->setPosition(
                        {coordinate_modified_command(i, 0), coordinate_modified_command(i, 1), coordinate_z});
            }
        }
        ...
        void set_goal(Eigen::Ref<EigenVec> goal_pos) {
            /// ERROR might happen when n_goal_set is 0!!!!
        ...
        void visualize_analytic_planner(Eigen::Ref<EigenRowMajorMat> planned_path) {
            ...
            if (path_length > max_analytic_planned_path_length) {
                ...
                for (int i = max_analytic_planned_path_length; i < path_length; i++) {
                    analytic_planned_traj.push_back(
                            server_->addVisualBox("analytic_planned_traj" + std::to_string(i), 0.15, 0.15, 0.15, 1, 1,
                                                  0));
                }
